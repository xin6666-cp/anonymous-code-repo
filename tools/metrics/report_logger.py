...
class ReportLogger(Metric):
    ...
    def update(self, generated_reports, dicom_ids=None, reference_reports=None):
        """
        Argument/s:
        ...
        assert isinstance(generated_reports, list), '"reports" must be a list of strings.'
        assert all(isinstance(i, str) for i in generated_reports), 'Each element of "reports" must be a string.'
        if reference_reports is None:
            for (i, j) in zip(generated_reports, dicom_ids):
                self.reports.append({'dicom_id': j, 'generated_report': i})
        else:
            for (i, j, l) in zip(generated_reports, dicom_ids, reference_reports):
                self.reports.append({'dicom_id': j, "reference_report": l, 'generated_report': i})
